...
\def\itemize{\par\bigskip\bgroup\leftskip 0.6in}
\def\item#1{\par\leavevmode
  \outline{#1}{1}\label{#1}\hbox to 0pt{\hss\hbox to 0.6in {#1\hfill}}\ignorespaces}
...
\section{DESCRIPTION}
       ...
       Hintview displays a binary HINT file.
       These files usually have the {\tt .hnt} extension.
...
\item{-l}     Start the viewer in "light" mode. In this mode the  viewer  will
              display  the  document  using  black ink on white background.
...
\item{-?}     Print help message and exit.
...
\item{Ctrl-D} Toggle between dark and light mode.
              See also the \link{-d} and \link{-l} option.
...
\item{Scroll} wheel (EXPERIMENTAL) With some implementations of HintView,
...
\section{NOTES}
       This is a program to render HINT files. Other  programs  to
       display  HINT  files  are  available  on  the HINT project home page at
       https://hint.userweb.mwn.de/ where you find viewers for the Windows,
       Linux, MacOS, iOS, and  Android.

       Currently  the  best  way to produce HINT files is the use of {\tt hitex},
%       or {\tt hilatex},
       versions of \TeX\ that produce HINT files as  output.  The  HINT
