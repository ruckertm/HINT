...
#define GLYPH_BIT 0x200000 \

#define is_otf_font(A) (font_def[f].m==0)  \

#define ft_glyph_id(F,C) (((C) &GLYPH_BIT) ?((C) &~GLYPH_BIT) :FT_Get_Char_Index(F,C) )  \

#define GCACHE_BITS 10
#define GCACHE_SIZE (1<<GCACHE_BITS) 
#define GCACHE_MASK (GCACHE_SIZE-1)  \

#define FIBONACCI_FACTOR 0x9F406BB9654C5A55 \
...
while(p!=null)
switch(type(p)){
case hlist_node:case vlist_node:case rule_node:/*236:*/
#line 4888 "hint.w"
...
0,
ft_size,
(ONE*72.27)/(double)font_def[f].hpxs,
(ONE*72.27)/(double)font_def[f].vpxs);
...
if(pk_data==NULL||font_def[f].pk_glyphs==NULL)return;
...
{unsigned int h= key*FIBONACCI_FACTOR;
h= (h>>(32-2*GCACHE_BITS))&GCACHE_MASK;
return h;
}
/*:663*//*664:*/
#line 13706 "hint.w"
