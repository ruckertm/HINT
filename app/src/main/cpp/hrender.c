...
static scaled max_h, max_v;

static scaled cur_h, cur_v;
static scaled rule_ht, rule_dp, rule_wd; /*size of current rule being output*/ 

static int cur_f; /* the current font number */ 
static font_ptr cur_pk; /* the current font pointer */ 
...
static void render_char(int x, int y, font_ptr pk, unsigned int s, unsigned int cc)
/* display the given charcter cc from font pk at x, y at size s  */
{ double w, h, dx, dy, m;
  gcache_t *g=hget_glyph(pk,cc);
  if (g==NULL) return;
  /* account for scaled fonts */
  if (s==0) m=1.0;
  else m=((double)s/(double)(1<<16))/pk->ds; /* s is a scaled by 1<<16 but ds is scaled by 1<<20 */
  dx=((double)g->hoff/pk->hppp)*m;
  dy=((double)g->voff/pk->vppp)*m;
  w =((double)g->w/pk->hppp)*m;
  h =((double)g->h/pk->vppp)*m;
...
/* render the image found in segment n at x,y 
   scaled to fit into the boundary defined by w and h.
   */
{ 
...
{ reswitch:
  ...
  { do 
	{ uint8_t f= font(p);
	  ...
	  render_char(cur_h, cur_v, cur_pk, font_def[cur_f].s,c);
		...
		{ scaled h,w;
	      ...
	      { if(g_sign==stretching)
	        { if(image_stretch_order(p)==g_order)
	          { vet_glue(float(glue_set(this_box))*image_stretch(p));
       		    w=w+round(glue_temp);
	          }
	        }
	        else if(image_shrink_order(p)==g_order)
	        { vet_glue(float(glue_set(this_box))*image_shrink(p));
	          w=w-round(glue_temp);
	        }
	      }
		  if (w!=image_width(p))
...
{
  if (height(p)+depth(p)> max_v) max_v= height(p)+depth(p);
  if (width(p)> max_h) max_h= width(p);

  cur_h= 0;
  cur_v= height(p);
  cur_f=-1;cur_pk=NULL; /* needs to be reset when we read in a new file */
