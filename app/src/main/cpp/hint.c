...
static void hget_font_params(uint8_t n, font_def_t *f) {
    HGET16(f->m);
    RNG("Font metrics", f->m, 3, max_section_no);
    HGET16(f->q);
    RNG("Font glyphs", f->q, 3, max_section_no);
    f->g = hget_glue_node();
...
#line 763 "hint.w"
        ...
        if (k != penalty_kind && k != kern_kind && k != ligature_kind &&
            k != hyphen_kind && k != glue_kind && k != math_kind && k != rule_kind &&
            k != image_kind)
...
#line 890 "hint.w"
    ...
    if (t != s)
        QUIT("List sizes at "
                     ...
                     " and "
                     SIZE_F
                     " do not match 0x%x != 0x%x", list_start - hstart, list_end - hstart, s, t);
    return p;
}
...
void hset_param_list(param_def_t *p) {
    ...
    while (p != NULL) {
        ...
        if (q->k == int_kind) {
            ...
            q->g = pointer_def[glue_kind][q->n];
            pointer_def[glue_kind][q->n] = p->p.g;
        }
        p = p->next;
    }
}
...
void hrestore_param_list(void) {
    while (par_save_ptr > 0) {
        param_t *q;
        q = &(par_save[--par_save_ptr]);
        if (q->k == int_kind) { integer_def[q->n] = q->i; }
        else if (q->k == dimen_kind) { dimen_def[q->n] = q->d; }
        else if (q->k == glue_kind) { pointer_def[glue_kind][q->n] = q->g; }
    }
    par_save_ptr = 0;
}
/*:54*//*58:*/
...
static void hget_range_def(uint8_t a, uint8_t pg) {
    ...
    REF(page_kind, pg);
    REF(range_kind, n);
    if (INFO(a) & b100) { if (INFO(a) & b001)HGET32(f); else HGET16(f); }
    else f = 0;
    if (INFO(a) & b010) { if (INFO(a) & b001)HGET32(t); else HGET16(t); }
    else t = 0xFFFFFFFF;
    range_def[n].pg = pg;
    range_def[n].f = f;
    range_def[n].t = t;
...
static scaled hget_xdimen(uint8_t a) {
    ...
    switch (a) {
        case TAG(xdimen_kind, b000):
            return hget_xdimen_ref(HGET8);
        case TAG(xdimen_kind, b001): HGET_XDIMEN(b001, x);
            break;
        case TAG(xdimen_kind, b010): HGET_XDIMEN(b010, x);
            break;
        case TAG(xdimen_kind, b011): HGET_XDIMEN(b011, x);
            break;
        case TAG(xdimen_kind, b100): HGET_XDIMEN(b100, x);
            break;
        case TAG(xdimen_kind, b101): HGET_XDIMEN(b101, x);
            break;
        case TAG(xdimen_kind, b110): HGET_XDIMEN(b110, x);
            break;
        case TAG(xdimen_kind, b111): HGET_XDIMEN(b111, x);
            break;
        default:
            x.w = 0;
            x.h = x.v = 0.0;
...
scaled hteg_xdimen(uint8_t a) {
    ...
    switch (a) {
        case TAG(xdimen_kind, b000):
            return hget_xdimen_ref(HTEG8);
        case TAG(xdimen_kind, b001):
        HTEG_XDIMEN(b001, x);
            break;
        case TAG(xdimen_kind, b010):
        HTEG_XDIMEN(b010, x);
            break;
        case TAG(xdimen_kind, b011):
        HTEG_XDIMEN(b011, x);
            break;
        case TAG(xdimen_kind, b100):
        HTEG_XDIMEN(b100, x);
            break;
        case TAG(xdimen_kind, b101):
        HTEG_XDIMEN(b101, x);
            break;
        case TAG(xdimen_kind, b110):
        HTEG_XDIMEN(b110, x);
            break;
        case TAG(xdimen_kind, b111):
        HTEG_XDIMEN(b111, x);
            break;
        default:
            x.w = 0;
...
static void hget_size_boundary(info_t info) {
    ...
    n = HGET8;
    if (n - 1 != 0x100 - info)
        QUIT("Size boundary byte 0x%x with info value %d at "
                     SIZE_F,
...
#line 1710 "hint.w"
        ...
        t = hget_list_size(INFO(a));
        if (t != s)
            QUIT("List sizes at 0x%x and "
                         SIZE_F
                         " do not match 0x%x != 0x%x", node_pos + 1, hpos - hstart - s - 1, s, t);
/*66:*/
#line 1116 "hint.w"
...
static pointer hteg_hbox_node(void) {/*69:*/
#line 1172 "hint.w"
...
static pointer hteg_vbox_node(void) {/*69:*/
#line 1172 "hint.w"
...
static pointer hget_hyphen_node(void) {/*65:*/
#line 1111 "hint.w"
...
#line 133 "hint.w"
    ...
    switch (a) {
        case TAG(0, b000 + 0): HGET_ENTRY(b000 + 0, *e);
            break;
        case TAG(0, b000 + 1): HGET_ENTRY(b000 + 1, *e);
            break;
        case TAG(0, b000 + 2): HGET_ENTRY(b000 + 2, *e);
            break;
        case TAG(0, b000 + 3): HGET_ENTRY(b000 + 3, *e);
            break;
        case TAG(0, b100 + 0): HGET_ENTRY(b100 + 0, *e);
            break;
        case TAG(0, b100 + 1): HGET_ENTRY(b100 + 1, *e);
            break;
        case TAG(0, b100 + 2): HGET_ENTRY(b100 + 2, *e);
            break;
        case TAG(0, b100 + 3): HGET_ENTRY(b100 + 3, *e);
            break;
        default:
            TAGERR(a);
            break;
...
#line 504 "hint.w"
    ...
    for (k = 0; k < 32; k++) {
        if (k == font_kind || k == int_kind || k == dimen_kind || k == xdimen_kind ||
            k == glue_kind || k == baseline_kind || k == range_kind || k == page_kind ||
            k == param_kind)
            continue;
        if (max_ref[k] >= 0 && max_ref[k] <= 256) {
            DBG(dbgdef, "Allocating Definitions for %s=%d: %d entries of "
                    SIZE_F
...
pointer hset_glue(glue_t *g) {
    if (ZERO_GLUE(*g))
        ...
        width(p) = g->w.w;
        stretch(p) = round(g->p.f * ONE);
        stretch_order(p) = g->p.o;
        shrink(p) = round(g->m.f * ONE);
        shrink_order(p) = g->m.o;
        return p;
    }
}
...
void hset_default_definitions(void) {
    ...
    for (i = 0; i <= MAX_GLUE_DEFAULT; i++)
        pointer_def[glue_kind][i] = hset_glue(&(glue_defaults[i]));
...
uint32_t hget_utf8(void) {
    ...
    if (a < 0x80)return a;
        ...
        if ((a & 0xE0) == 0xC0) {
            ...
            QUIT("UTF8 byte sequence expected");
    }
}
/*:74*//*100:*/
#line 1518 "hint.w"
...
pointer hget_hbox_node(void) {/*65:*/
#line 1111 "hint.w"
...
#line 1400 "hint.w"
        ...
        case TAG(glyph_kind, 1): HTEG_GLYPH(1);
            break;
        case TAG(glyph_kind, 2): HTEG_GLYPH(2);
            break;
        case TAG(glyph_kind, 3): HTEG_GLYPH(3);
            break;
        case TAG(glyph_kind, 4): HTEG_GLYPH(4);
            break;
/*:86*//*89:*/
#line 1420 "hint.w"

        case TAG(penalty_kind, 0): tail_append(new_penalty(hget_integer_ref(HTEG8)));
            break;
        case TAG(penalty_kind, 1): {
            tail_append(new_penalty(HTEG8));
        }
            break;
        case TAG(penalty_kind, 2): {
            int16_t n;
            HTEG16(n);
            RNG("Penalty", n, -20000, +20000);
            tail_append(new_penalty(n));
        }
            break;
/*:89*//*94:*/
#line 1465 "hint.w"

        case TAG(math_kind, b000): tail_append(hget_math_ref(HTEG8));
            break;
        case TAG(math_kind, b100): HTEG_MATH(b100, m);
            break;
        case TAG(math_kind, b010): HTEG_MATH(b010, m);
            break;
        case TAG(math_kind, b101): HTEG_MATH(b101, m);
            break;
        case TAG(math_kind, b011): HTEG_MATH(b011, m);
            break;
/*:94*//*99:*/
#line 1508 "hint.w"

        case TAG(rule_kind, b000): tail_append(hget_rule_ref(HTEG8));
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b011): HTEG_RULE(b011);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b101): HTEG_RULE(b101);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b001): HTEG_RULE(b001);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b110): HTEG_RULE(b110);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b111): HTEG_RULE(b111);
            prev_depth = ignore_depth;
            break;
/*:99*//*106:*/
#line 1579 "hint.w"

        case TAG(glue_kind, b000): tail_append(new_glue(hget_glue_ref(HTEG8)));
            break;
        case TAG(glue_kind, b001): {
            pointer p;
            HTEG_GLUE(b001);
            tail_append(new_glue(p));
        }
            break;
...
#line 2031 "hint.w"
        ...
        case TAG(hset_kind, b000): {
            pointer p;
            HTEG_SET(b000);
            hset(p, sto, st, sho, sh, x);
            happend_to_vlist(p);
        }
            break;
        case TAG(hset_kind, b001): {
            pointer p;
            HTEG_SET(b001);
            hset(p, sto, st, sho, sh, x);
            happend_to_vlist(p);
        ...
        case TAG(vset_kind, b000): {
            pointer p;
            HTEG_SET(b000);
            vset(p, sto, st, sho, sh, x);
            happend_to_vlist(p);
        }
            break;
        case TAG(vset_kind, b001): {
            pointer p;
            HTEG_SET(b001);
            vset(p, sto, st, sho, sh, x);
            happend_to_vlist(p);
...
#line 2160 "hint.w"
        ...
        case TAG(hpack_kind, b000): {
            HTEG_PACK(b000);
            p = hpack(p, x, m);
            happend_to_vlist(p);
        }
            break;
        case TAG(hpack_kind, b010): {
            HTEG_PACK(b010);
            p = hpack(p, x, m);
            happend_to_vlist(p);
        }
            break;
        case TAG(hpack_kind, b100): {
            HTEG_PACK(b100);
            p = hpack(p, x, m);
            happend_to_vlist(p);
        }
            break;
        case TAG(hpack_kind, b110): {
            HTEG_PACK(b110);
            p = hpack(p, x, m);
            happend_to_vlist(p);
        }
        ...
        case TAG(vpack_kind, b000): {
            HTEG_PACK(b000);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b010): {
            HTEG_PACK(b010);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b100): {
            HTEG_PACK(b100);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b110): {
            HTEG_PACK(b110);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b001): {
            HTEG_PACK(b001);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b011): {
            HTEG_PACK(b011);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b101): {
            HTEG_PACK(b101);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
            break;
        case TAG(vpack_kind, b111): {
            HTEG_PACK(b111);
            p = vpackage(p, x, m, d);
            happend_to_vlist(p);
        }
...
#line 2215 "hint.w"
        ...
        case TAG(kern_kind, b001): {
            ...
            break;
        case TAG(kern_kind, b010): {
            HTEG_KERN(b010);
        }
            break;
        case TAG(kern_kind, b011): {
            HTEG_KERN(b011);
        }
            break;
        case TAG(kern_kind, b100): {
            HTEG_KERN(b100);
        }
            break;
        case TAG(kern_kind, b101): {
            ...
            break;
        case TAG(kern_kind, b110): {
            HTEG_KERN(b110);
        }
            break;
        case TAG(kern_kind, b111): {
            HTEG_KERN(b111);
        }
            break;
/*:135*//*139:*/
#line 2255 "hint.w"
        ...
        case TAG(leaders_kind, 0): tail_append(hget_leaders_ref(HTEG8));
            break;
        case TAG(leaders_kind, 1): HTEG_LEADERS(1);
            break;
        case TAG(leaders_kind, 2): HTEG_LEADERS(2);
            break;
        case TAG(leaders_kind, 3): HTEG_LEADERS(3);
            break;
/*:139*//*143:*/
#line 2293 "hint.w"

        case TAG(baseline_kind, b000): {
            hget_baseline_ref(HTEG8);
            cur_list.bs_pos = hpos - 1;
        }
            break;
...
#line 2344 "hint.w"
        ...
        case TAG(ligature_kind, 0): tail_append(hget_ligature_ref(HTEG8));
            break;
        case TAG(ligature_kind, 1): HTEG_LIG(1);
            break;
        case TAG(ligature_kind, 2): HTEG_LIG(2);
            break;
        case TAG(ligature_kind, 3): HTEG_LIG(3);
            break;
        case TAG(ligature_kind, 4): HTEG_LIG(4);
            break;
        case TAG(ligature_kind, 5): HTEG_LIG(5);
            break;
        case TAG(ligature_kind, 6): HTEG_LIG(6);
            break;
        case TAG(ligature_kind, 7): HTEG_LIG(7);
            break;
/*:147*//*151:*/
#line 2393 "hint.w"

        case TAG(hyphen_kind, b000): tail_append(hget_hyphen_ref(HTEG8));
            break;
...
#line 2752 "hint.w"
        ...
        case TAG(par_kind, b000):
            hteg_paragraph(b000);
            break;
        case TAG(par_kind, b010):
            hteg_paragraph(b010);
            break;
        case TAG(par_kind, b100):
            hteg_paragraph(b100);
            break;
        case TAG(par_kind, b110):
            hteg_paragraph(b110);
            break;
/*:162*//*170:*/
#line 2862 "hint.w"

        case TAG(display_kind, b000): HTEG_DISPLAY(b000);
            break;
        case TAG(display_kind, b001): HTEG_DISPLAY(b001);
            break;
        case TAG(display_kind, b010): HTEG_DISPLAY(b010);
            break;
        case TAG(display_kind, b100): HTEG_DISPLAY(b100);
            break;
        case TAG(display_kind, b101): HTEG_DISPLAY(b101);
            break;
        case TAG(display_kind, b110): HTEG_DISPLAY(b110);
            break;
/*:170*//*176:*/
#line 2945 "hint.w"

        case TAG(image_kind, b000):
            hget_image_ref(HTEG8);
            break;
        case TAG(image_kind, b100): HTEG_IMAGE(b100);
            break;
        case TAG(image_kind, b101): HTEG_IMAGE(b101);
            break;
        case TAG(image_kind, b110): HTEG_IMAGE(b110);
            break;
        case TAG(image_kind, b111): HTEG_IMAGE(b111);
            break;
/*:176*/
#line 1193 "hint.w"
...
pointer hteg_hyphen_node(void) {/*69:*/
#line 1172 "hint.w"
...
#line 1394 "hint.w"
        ...
        case TAG(glyph_kind, 1): HGET_GLYPH(1);
            break;
        case TAG(glyph_kind, 2): HGET_GLYPH(2);
            break;
        case TAG(glyph_kind, 3): HGET_GLYPH(3);
            break;
        case TAG(glyph_kind, 4): HGET_GLYPH(4);
            break;
/*:85*//*88:*/
#line 1414 "hint.w"

        case TAG(penalty_kind, 0): tail_append(new_penalty(hget_integer_ref(HGET8)));
            break;
        case TAG(penalty_kind, 1): {
            tail_append(new_penalty(HGET8));
        }
            break;
        case TAG(penalty_kind, 2): {
            int16_t n;
...
#line 1458 "hint.w"
        ...
        case TAG(math_kind, b000): tail_append(hget_math_ref(HGET8));
            break;
        case TAG(math_kind, b100): HGET_MATH(b100, m);
            break;
        case TAG(math_kind, b010): HGET_MATH(b010, m);
            break;
        case TAG(math_kind, b101): HGET_MATH(b101, m);
            break;
        case TAG(math_kind, b011): HGET_MATH(b011, m);
            break;
/*:93*//*98:*/
#line 1499 "hint.w"

        case TAG(rule_kind, b000): tail_append(hget_rule_ref(HGET8));
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b011): HGET_RULE(b011);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b101): HGET_RULE(b101);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b001): HGET_RULE(b001);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b110): HGET_RULE(b110);
            prev_depth = ignore_depth;
            break;
        case TAG(rule_kind, b111): HGET_RULE(b111);
            prev_depth = ignore_depth;
            break;
/*:98*//*105:*/
#line 1567 "hint.w"

        case TAG(glue_kind, b000): tail_append(new_glue(hget_glue_ref(HGET8)));
            break;
        case TAG(glue_kind, b001): {
            pointer p;
            HGET_GLUE(b001);
...
#line 2249 "hint.w"
        ...
        case TAG(leaders_kind, 0): tail_append(hget_leaders_ref(HGET8));
            break;
        case TAG(leaders_kind, 1): HGET_LEADERS(1);
            break;
        case TAG(leaders_kind, 2): HGET_LEADERS(2);
            break;
        case TAG(leaders_kind, 3): HGET_LEADERS(3);
            break;
/*:138*//*142:*/
#line 2283 "hint.w"
...
#line 2333 "hint.w"
        ...
        case TAG(ligature_kind, 0): tail_append(hget_ligature_ref(HGET8));
            break;
        case TAG(ligature_kind, 1): HGET_LIG(1);
            break;
        case TAG(ligature_kind, 2): HGET_LIG(2);
            break;
        case TAG(ligature_kind, 3): HGET_LIG(3);
            break;
        case TAG(ligature_kind, 4): HGET_LIG(4);
            break;
        case TAG(ligature_kind, 5): HGET_LIG(5);
            break;
        case TAG(ligature_kind, 6): HGET_LIG(6);
            break;
        case TAG(ligature_kind, 7): HGET_LIG(7);
            break;
/*:146*//*150:*/
#line 2383 "hint.w"

        case TAG(hyphen_kind, b000): tail_append(hget_hyphen_ref(HGET8));
            break;
        case TAG(hyphen_kind, b001): {
            HGET_HYPHEN(b001);
            tail_append(p);
...
#line 2444 "hint.w"
        ...
        case TAG(par_kind, b000): HGET_PAR(b000);
            break;
        case TAG(par_kind, b010): HGET_PAR(b010);
            break;
        case TAG(par_kind, b100): HGET_PAR(b100);
            break;
        case TAG(par_kind, b110): HGET_PAR(b110);
            break;
/*:155*//*169:*/
#line 2853 "hint.w"

        case TAG(display_kind, b000): HGET_DISPLAY(b000);
            break;
        case TAG(display_kind, b001): HGET_DISPLAY(b001);
            break;
        case TAG(display_kind, b010): HGET_DISPLAY(b010);
            break;
        case TAG(display_kind, b100): HGET_DISPLAY(b100);
            break;
        case TAG(display_kind, b101): HGET_DISPLAY(b101);
            break;
        case TAG(display_kind, b110): HGET_DISPLAY(b110);
            break;
/*:169*//*172:*/
#line 2902 "hint.w"

        case TAG(adjust_kind, 1): HGET_ADJUST(1);
            break;
        case TAG(adjust_kind, 2): HGET_ADJUST(2);
            break;
        case TAG(adjust_kind, 3): HGET_ADJUST(3);
            break;
        case TAG(adjust_kind, 4): HGET_ADJUST(4);
            break;
        case TAG(adjust_kind, 5): HGET_ADJUST(5);
            break;
/*:172*//*175:*/
#line 2938 "hint.w"

        case TAG(image_kind, b000):
            hget_image_ref(HGET8);
            break;
        case TAG(image_kind, b100): HGET_IMAGE(b100);
            break;
        case TAG(image_kind, b101): HGET_IMAGE(b101);
            break;
        case TAG(image_kind, b110): HGET_IMAGE(b110);
            break;
        case TAG(image_kind, b111): HGET_IMAGE(b111);
            break;
/*:175*/
#line 1130 "hint.w"
...
#line 3394 "hint.w"
            ...
            case penalty_node:
                if (page_contents == empty)goto done1;
                else pi = penalty(p);
                break;
            case mark_node:
                goto contribute;
...
#line 3398 "hint.w"
            ...
            default:
                MESSAGE("Unexpected node type %d in build_page_up ignored\n", type(p));
        }
        if (pi < inf_penalty)
/*206:*/
#line 3498 "hint.w"
...
r= link(r);
            ...
            if ((c == awful_bad) || (pi <= eject_penalty)) {/*207:*/
#line 3525 "hint.w"
...
#line 3537 "hint.w"
                ...
                while (true) {
                    ...
                    if (q == null)return false;
                        ...
                        break;
                    else if (type(q) == penalty_node || type(q) == glue_node ||
                             type(q) == kern_node) {
                        link(page_head) = link(q);
                        link(q) = null;
                        flush_node_list(q);
